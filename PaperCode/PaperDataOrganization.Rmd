...
# # get labelled dataset only for SKCM with batches and subtypes
# get_merge_df <- function(cancer, df_count){
#   # This function merge all the meta data with the count table by patient or Sample
#   # @params: cancer - name of cancer, eg "SKCM", make sure that are subtype infromation and TCGA-cancer_batches.tsv in ../RealData/
#   # @params: df_count - count df with last column is the patient
#   # @return: meta - all the patient information including subtype, clinical and batch rows of meta are consistent with rows of df_count (or mat_count from get_count_df)
#   
#   meta_batch <- read.table(paste("../RealData/TCGA-",cancer,"_batches.tsv", sep=""), sep = '\t', header = TRUE)
#   meta_batch$Sample <- gsub("-", ".", meta_batch$Sample)
#   meta_clinic <- GDCquery_clinic(paste("TCGA-", cancer, sep=""), type = "clinical", save.csv = FALSE)
#   meta_clinic$patient <-  meta_clinic$submitter_id
#   meta_subtype <- TCGAquery_subtype(tumor = cancer)
#   meta_subtype <- meta_subtype[meta_subtype$ALL_SAMPLES == "Yes",]
#   merge_df <- df_count %>% 
#     left_join(meta_batch, by = "Sample") %>%
#     left_join(meta_clinic, by = "patient") %>%
#     left_join(meta_subtype, by = "patient")
#   
#   meta <- merge_df[,(ncol(df_count)-2):ncol(merge_df)]
#   return(meta)
# }
# get_filter_df <- function(mat_count, meta, var_name, use_levels){
#   # This funciton filter the count data frame by variable var_name with levels use_levels
#   # @params: mat_count - count data frame with rows are samples
#   # @params: meta - data frame with meta information of samples, consistent with mat_count samples.
#   # @params: var_name - character of variable in meta you want to use as gorups in differential expression analysis.
#   # @params: use_levels -  levels of variable var_name you want to use as groups in differential expression analysis.
#   # @return: a list contating the filtered count data frame filter_count and groups vector groups.
#   variable <- meta[,var_name]
#   keep <- variable %in% use_levels
#   groups <- factor(variable[keep])
#   filter_count <- mat_count[keep, ]
#   return(list(filter_count =  filter_count, groups = groups))
# }
# 
...
<!-- ### "SKCMLAMLPositivePoly_thres.csv": (452+188)*(266+1+1) with groups labels cancer type positive control genes using threshold thres using polycistronic clusters  -->
<!-- ```{r, Generate SKCMlAMLPositivePoly_thres.csv, eval=F } -->
<!-- thres <- 1 -->
<!-- coords <- read.csv("../RealData/miRBase_coords.csv") -->
<!-- count_list_1 <- get_count_df(cancer = "SKCM") -->
<!-- count_list_2 <- get_count_df(cancer = "LAML") -->
<!-- count_list <- get_combine_count_df(df_count_1 = count_list_1$df_count, -->
<!--                                    f_count_2 = count_list_2$df_count) -->
<!-- count_list_pos <- get_count_positive_df(count_list = count_list, thres = thres, poly = T, coords = coords) -->
<!-- write.table(data.frame(count_list_pos$df_count, samples = rownames(count_list_pos$df_count)), -->
<!--             paste("../RealData/SKCMLAMLPositivePoly_", thres, ".csv", sep = ""), -->
<!--             sep = ",", row.names = F, col.names = T) -->
<!-- ``` -->
...
<!-- # PRAD BRCA combine datasets generation -->

<!-- ### "BRCAPRAD.csv": (1207+551)*(1881+1+1) with group labels cancer type -->
<!-- ```{r, Generate BRCAPRAD.csv, eval=F} -->
<!-- count_list_1 <- get_count_df(cancer = "BRCA") -->
<!-- count_list_2 <- get_count_df(cancer = "PRAD") -->
<!-- count_list <- get_combine_count_df(df_count_1 = count_list_1$df_count, -->
<!--                                    df_count_2 = count_list_2$df_count, -->
<!--                                    group_name = c("BRCA","PRAD")) -->
<!-- # evaluate the major variation -->
<!-- mat <- as.matrix(count_list$mat_count) -->
<!-- UMAP_fit = umap::umap(mat) -->
<!-- UMAP_df = UMAP_fit$layout %>%  -->
<!--              as.data.frame() %>% -->
<!--              rename(UMAP1 = "V1", UMAP2 = "V2") %>% -->
<!--              mutate(ID = row_number()) -->
<!-- factor_df = data.frame(groups = count_list$df_count$groups) %>% mutate(ID=row_number()) -->
<!-- plot_df = UMAP_df %>% inner_join(factor_df, by="ID") -->
<!-- plot_df %>% ggplot(aes(x = UMAP1, y = UMAP2, color = groups))+ -->
<!--                   geom_point()+ -->
<!--                   theme(legend.position="bottom")+ -->
<!--                   labs(title="UMAP plot of BRCAPRAD combined samples")+ -->
<!--                   theme_bw() -->

<!-- write.table(data.frame(count_list$df_count, samples = rownames(count_list$df_count)),  -->
<!--             "../RealData/BRCAPRAD.csv", -->
<!--             sep = ",", row.names = F, col.names = T) -->
<!-- ``` -->
...
<!-- count_list_pos <- get_count_positive_df(count_list = count_list, thres = thres) -->
<!-- # evaluate the major variation -->
<!-- mat <- as.matrix(count_list_pos$mat_count) -->
<!-- UMAP_fit = umap::umap(mat) -->
<!-- UMAP_df = UMAP_fit$layout %>%  -->
<!--              as.data.frame() %>% -->
<!--              rename(UMAP1 = "V1", UMAP2 = "V2") %>% -->
<!--              mutate(ID = row_number()) -->
<!-- factor_df = data.frame(groups = count_list_pos$df_count$groups) %>% mutate(ID=row_number()) -->
<!-- plot_df = UMAP_df %>% inner_join(factor_df, by="ID") -->
<!-- plot_df %>% ggplot(aes(x = UMAP1, y = UMAP2, color = groups))+ -->
<!--                   geom_point()+ -->
<!--                   theme(legend.position="bottom")+ -->
<!--                   labs(title="UMAP plot of BRCAPRAD combined samples\n with positive control genes thres 3")+ -->
<!--                   theme_bw() -->
<!-- write.table(data.frame(count_list_pos$df_count, samples = rownames(count_list_pos$df_count)), -->
...
<!-- # ACC RNA-seq dataset generation -->

<!-- ### "ACCPositive_12.csv": 79 * (1389 + 1): with samples, no groups #### -->
<!-- ```{r} -->
<!-- ACC <- read.csv("../RealData/TCGA-ACC.csv", header = T) -->
<!-- RC_present <- ACC[, 2:dim(ACC)[2]] -->
<!-- genes_present <- ACC[, 1] -->
<!-- # expression level: 12 mean gives 1389 genes -->
...
<!-- RC_we <- as.data.frame(t(RC_we)) -->
<!-- colnames(RC_we) <- genes_we -->
<!-- write.table(data.frame(RC_we, samples = colnames(ACC)[2:80]), -->
<!--             "../RealData/ACCPositive_12.csv", sep =",", row.names = F, col.names = T) -->

<!-- ``` -->
<!-- ### "ACCPositive_12_NormMethod.csv": 3 * 79 * (1389 + 1) using normalized data -->
<!-- ```{r, Generate SKCM_NormMethod.csv, eval=F} -->
<!-- selected <- c("TC", "TMM","UQ") -->
<!-- dat <- read.csv("../RealData/ACCPositive_12.csv", header =  T) -->
<!-- count <- t(select_if(dat, is.numeric)) -->
<!-- genes <- colnames(dat)[colnames(dat) != 'samples'] -->
<!-- samples <- dat$samples -->
<!-- colnames(count) <- samples -->
<!-- rownames(count) <- genes -->
<!-- groups <- rep("Single", ncol(count)) -->
<!-- count_normed <- DANA::applyNormalization(data = count, -->
<!--                                          groups = groups, method = selected) -->
<!-- sel_normed <- count_normed[selected] -->
<!-- for (dat_ind in 1:length(selected)) { -->
<!--   norm_name <- selected[dat_ind] -->
<!--   sel_normed[[dat_ind]] <- data.frame(t(sel_normed[[dat_ind]]), samples = samples) -->
<!--   write.table(sel_normed[[dat_ind]], -->
<!--               paste("../RealData/ACCPositive_12", "_", selected[dat_ind], -->
<!--                     ".csv", sep = ""),  -->
<!--               sep =",", row.names = F, col.names = T) -->
<!-- } -->
